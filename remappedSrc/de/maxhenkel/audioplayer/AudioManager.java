...
public class AudioManager {
    ...
    public static WorldSavePath AUDIO_DATA = new WorldSavePath("audio_player_data");
    ...
    @Nullable
    public static UUID play(ServerWorld level, BlockPos pos, PlayerType type, CustomSound sound, @Nullable PlayerEntity player) {
        ...
        if (sound.isStaticSound() && AudioPlayer.SERVER_CONFIG.allowStaticAudio.get()) { //TODO Move option
            channelID = PlayerManager.instance().playStatic(
                    api,
                    level,
                    new Vec3d(pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D),
                    sound.getSoundId(),
                    (player instanceof ServerPlayerEntity p) ? p : null,
                    range,
                    type.getCategory(),
